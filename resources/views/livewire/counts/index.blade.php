                    ...
                    <div class="px-2 ">
                        ...
                        <div class=' bg-white rounded'>
                          <table class=' w-full '>
                            ...
                            <tbody>
                                @foreach ($getcomptes as $comptes)    
                                    <tr class='hover:bg-slate-50 border-t h-5 border-slate-200'>                    
                                    <td>
                                      <span class='text-xs text-left font-semibold px-2 text-[#141B27] flex justify-start gap-1 py-2'>{{ $comptes->nom_avec}}</span>
                                    </td>
                                    <td class='text-xs text-left font-semibold  text-[#141B27] pb-1'>
                                      <span class='pb-2'>{{ $comptes->nombre_avec}}</span>
                                    </td>
                                    <td class='text-xs text-left font-semibold  text-[#141B27] pb-1'>
                                      <span class='pb-2 text-center'>{{ $comptes->montantmax_avec}} $</span>
                                    </td>
                                    <td class='text-xs text-left font-semibold  text-[#141B27] pb-1'>
                                      <span class='pb-2'>{{ $comptes->nombrecycle_avec}}</span>
                                    </td>
                                    <td class='text-xs text-right font-normal  text-[#141B27] '>
                                      <div class="flex gap-2">
                                                              ...
                                                              <form wire:submit="ajouterutilisateur"  method="POST" enctype="multipart/form-data">
                                                                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-4">
                                                                  <div class="md:col-span-2">
                                                                    ...
                                                                    <input type="text" wire:input.debounce.2000ms="edit_name({{$comptes->id}}, $event.target.value)" value="{{$comptes->nom_avec}}" class="h-10 border mt-1 rounded-sm text-black px-2 w-full focus:outline-none" />
                                                                  </div>
                                                                  <div class="md:col-span-2">
                                                                    <label class="text-black" for="email">Nombre maximum de membre</label>
                                                                    <input type="text" wire:input.debounce.2000ms="edit_nombremembres({{$comptes->id}}, $event.target.value)" value="{{$comptes->nombre_avec}}" class="h-10 border mt-1 rounded-sm text-black px-2 w-full focus:outline-none" />
                                                                  </div>
                                                                  <div class="md:col-span-2">
                                                                    <label class="text-black" for="city">Montant maximum</label>
                                                                    <input type="text" wire:input.debounce.2000ms="edit_montantmaxi({{$comptes->id}}, $event.target.value)" value="{{ $comptes->montantmax_avec }}" class="h-10 border mt-1 rounded-sm text-black px-2 w-full focus:outline-none" />
                                                                  </div>
                                                                  <div class="md:col-span-2">
                                                                    <label class="text-black" for="city">Nombre de cycle</label>
                                                                    <input type="text" wire:input.debounce.2000ms="edit_cycles({{$comptes->id}}, $event.target.value)" value="{{ $comptes->nombrecycle_avec }}" class="h-10 border mt-1 rounded-sm text-black px-2 w-full focus:outline-none" />
                                                                  </div>
                                                                  <div class="md:col-span-2">
                                                                    <label class="text-black" for="city" for="city">Description</label>
                                                                    <textarea wire:input.debounce.2000ms="edit_description({{$comptes->id}}, $event.target.value)" value="{{ $comptes->description_avec }}" class=" border mt-1 rounded-sm text-black px-2  focus:outline-none"  placeholder=" Decrivez votre AVEC" name="" id="" cols="74" rows="5"></textarea>
                                                                  </div>
                                                                </div>
                                                              </form>
                                                            </div>
                                        ...
                                        <div wire:click="deleteuser({{ $comptes->id }})" wire:confirm="Permission Deleted successfully" class="w-7 h-7 rounded-full bg-slate-200 text-center  py-1.5  hover:bg-cyan-500 hover:text-white cursor-pointer transition ease-in-out duration-500">
                                        <i class="fa fa-trash"></i>
                                        </div>
                                        <div  wire:click="showdetailuserdata({{ $comptes->id}})" class="w-7 h-7 rounded-full bg-slate-200 text-center  py-1.5  hover:bg-cyan-500 hover:text-white cursor-pointer transition ease-in-out duration-500" >
                                        <i class="fa fa-eye"></i>
                                        </div>
                                      </div>
                                    </td>                                                   
                    ...
                    <div class="bg-white py-0">
                      ...
                      <div class=" grid grid-cols-4 gap-1">
                        <div class="pt-2">
                          ...
                          <div class="py-2 px-2">
                            <h1 class="text-left text-blue-400 font-bold text-lg">{{ $userdetaildata->$comptes}}</h1>
                          </div>
                        </div>
